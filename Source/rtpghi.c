...
{
    return (LTFAT_REAL) ( in - 2.0 * M_PI * ltfat_round(in / (2.0 * M_PI)) );
}
                           ...
                           const LTFAT_COMPLEX cin[], double stretch, LTFAT_COMPLEX cout[])
{
    ...
    M2 = p->M / 2 + 1; W = p->W;
    asyn = p->a;
    aanaprev = asyn / p->stretch; // old stretch
    aananext = asyn / stretch; // new stretch
                        ...
                        LTFAT_REAL* fgrad)
{
    ...
    fgrad[0]      = phase[1] * stretch;
    fgrad[M2 - 1] = phase[M2 - 2] * stretch;
                        ...
                        double stretch, LTFAT_REAL* tgrad)
{
    ...
    LTFAT_REAL const1prev = 2.0 * M_PI * aanaprev / M;
    LTFAT_REAL const1next = 2.0 * M_PI * aananext / M;
