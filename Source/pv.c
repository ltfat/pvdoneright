...
LTFAT_NAME(rtpghi_processor_callback)(void* userdata,
                                      const LTFAT_COMPLEX* in, int UNUSED(M2), int UNUSED(W), LTFAT_COMPLEX* out)
{
                    ...
                    LTFAT_NAME(pv_state)** pout)
{
    ...
    ltfat_int M = 8192;
    ltfat_int gl = 4096;
    ltfat_int fifoSize = bufLenMax * stretchmax;
    ...
    LTFAT_NAME(rtdgtreal_processor_setcallback)( p->procstate,
            LTFAT_NAME(rtpghi_processor_callback), p);
                       ...
                       ltfat_int outLen, LTFAT_REAL* out[])
{
    ...
    status =
        LTFAT_NAME(rtdgtreal_processor_execute_gen)(p->procstate,
                in, inLen, chanNo, outLen, out);
                       ...
                       ltfat_int outLen, LTFAT_REAL* out)
{
    ...
    status =
        LTFAT_NAME(rtdgtreal_processor_execute_gen_compact)(p->procstate,
                in, inLen, chanNo, outLen, out);
